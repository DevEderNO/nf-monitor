...
export function UploadCertificates() {
  ...
  const handleSelectDirectories = useCallback(async () => {
    const filepaths: IDirectory[] = await window.ipcRenderer.invoke('select-directories-upload-certificates');
    if (filepaths) {
      dispatch({
  ...
  return (
    <div className="p-4 m-4 flex flex-col gap-4 border rounded-md flex-1">
      <div className="flex">
        <Button onClick={handleSelectDirectories}>Selecionar diretórios para envio de certificados</Button>
      </div>
